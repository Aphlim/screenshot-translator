...
- 学英文软件 / 看报错信息 / 游戏剧情,**不想离开当前窗口**去复制粘贴翻译(or用于应对无法复制粘贴的情况)
- 想用 **自己的 API key**(DeepSeek 很便宜!不到1分钱一次,可控)而不是某些工具的内置额度
- 不想让任何第三方扫描你的剪贴板和屏幕
...
## 关于作者
  这是我这个非计算机从业人员的人生第一次尝试!我很兴奋!!因为不懂代码的我通过vibe coding做出了一个好用的小工具!*(虽然尝试发布的时候才发现有很多大佬都已经做过类似的项目了T.T)*
  做这个工具的初衷是我在学习一款小众的工程软件时, 面对全英的界面不知所措, 只能频繁截图 → 发给ai → 再对照原文, 屏幕切来切去很不方便也很容易打断我的思路, 正好我同时也在接触学习ai的相关内容, 因此就尝试了自己做一个小工具出来!
  这真的是一个功能简单好用的小应用, 诚邀您试一下!给我一颗宝贵的小星星, 并留下您宝贵的建议~
